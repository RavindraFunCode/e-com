...
class Color extends MY_Controller {
	...
	public function index($id='')
	{
        ...
        if(!empty($id)){
	        ...
	        $data['edit_item']=$this->db->get_where($this->tbl_ecom_color, ['id'=>$id])->row();
	    }
      $data['all_category'] = [];
		$this->load->view('backend/include/header', $header);
		$this->load->view('backend/catalogue/color-save', $data);
	...
	public function all()
	{
    	...
    	$header['sub_title']='Home';
		$this->load->view('backend/include/header', $header);
		$this->load->view('backend/catalogue/color-list');
		$this->load->view('backend/include/footer');
	...
	public function save()
	{
	    ...
	    $data['name'] = strtolower(str_replace(' ', '', trim($data['name'], " ")));
	    $data['is_active'] = $this->input->post('is_active');

	    if(empty($data['name'])){
	      $result['status']=0;
	      $result['message']='Color Name is required';
	    ...
	    $check=getValueByColumn($this->tbl_ecom_color, 'id', $check_where);
	    if(!empty($check))
	    {
	      $result['status']=0;
	      $result['message']='Color already exist!';
	      echo json_encode($result);die;
	    }
	    ...
	    if(!empty($edit_id))
	    {
	      ...
	      $this->db->update($this->tbl_ecom_color, $data,['id'=>$edit_id]);
	      $result['status']=1;
	      $result['message']=$this->my_name.' update successful';
	    }else{
	      $this->db->insert($this->tbl_ecom_color, $data);
	      $result['status']=1;
	...
	{ 
	    ...
	    $query = " SELECT * FROM ".$this->tbl_ecom_color." WHERE id>0 ";
	    ...
	    foreach ($result as $key => $val) {
	      ...
	      $sub_array[] = ++$key;
	      $sub_array[] = $val->name;
	      // $sub_array[] = getValueByColumn($this->tbl_ecom_color, 'name', ['slug'=>$val->parent_cat]) ;
	      ...
	      <div>
	        ...
	        <a href="' . base_url() . 'dashboard/catalogue/color/index/' .encryptor('encrypt',  $val->id) . '" class="btn btn-xs btn-secondary" style="border-radius: 0px;"> <i class="fas fa-edit"></i></a>
	    ...
	    $output = array(
	             ...
	             "recordsTotal"     =>  $this->db->get_where($this->tbl_ecom_color)->num_rows(),
	...
	public function deleteitem() {
      ...
      $row=$this->db->delete($this->tbl_ecom_color, ['id'=>$delete_id]);
      if($row) {
          $result['status']=1;
