...
class Unit extends MY_Controller {
	...
	public function index($id='')
	{
        ...
        if(!empty($id)){
	        ...
	        $data['edit_item']=$this->db->get_where($this->tbl_ecom_unit, ['id'=>$id])->row();
	    }
      $data['all_category'] = [];
		$this->load->view('backend/include/header', $header);
		$this->load->view('backend/catalogue/unit-save', $data);
    ...
    $header['sub_title']='Home';
		$this->load->view('backend/include/header', $header);
		$this->load->view('backend/catalogue/unit-list');
		$this->load->view('backend/include/footer');
	...
	public function save()
	{
	    ...
	    $check=getValueByColumn($this->tbl_ecom_unit, 'id', $check_where);
	    if(!empty($check))
	    {
	      $result['status']=0;
	    ...
	    if(!empty($edit_id))
	    {
	      ...
	      $this->db->update($this->tbl_ecom_unit, $data,['id'=>$edit_id]);
	      $result['status']=1;
	      $result['message']=$this->my_name.' update successful';
	    }else{
	      $this->db->insert($this->tbl_ecom_unit, $data);
	      $result['status']=1;
	...
	{ 
	    ...
	    $query = " SELECT * FROM ".$this->tbl_ecom_unit." WHERE id>0 ";
	    ...
	    foreach ($result as $key => $val) {
	      ...
	      $sub_array[] = ++$key;
	      $sub_array[] = $val->name;
	      // $sub_array[] = getValueByColumn($this->tbl_ecom_unit, 'name', ['slug'=>$val->parent_cat]) ;
	      ...
	      <div>
	        ...
	        <a href="' . base_url() . 'dashboard/catalogue/unit/index/' .encryptor('encrypt',  $val->id) . '" class="btn btn-xs btn-secondary" style="border-radius: 0px;"> <i class="fas fa-edit"></i></a>
	    ...
	    $output = array(
	             ...
	             "recordsTotal"     =>  $this->db->get_where($this->tbl_ecom_unit)->num_rows(),
	...
	public function deleteitem() {
      ...
      $row=$this->db->delete($this->tbl_ecom_unit, ['id'=>$delete_id]);
      if($row) {
          $result['status']=1;
