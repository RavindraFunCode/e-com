...
function get_brand_by_slug($slug, $arg = false){
	$ci = &get_instance();
	$data = $ci->db->get_where('tbl_ecom_brand', ['slug'=>$slug])->row();
...
function get_category_by_slug($slug, $arg = false){
	$ci = &get_instance();
	$data = $ci->db->get_where('tbl_ecom_category', ['slug'=>$slug])->row();
...
function get_product_image_by_id($id){
	$ci = &get_instance();
	return $ci->db->get_where('tbl_ecom_product_image', ['product_id'=>$id])->result();
}
function get_product_color_by_ids($ids){
	$ci = &get_instance();
	$ci->db->where_in('id', $ids);
	return $ci->db->get('tbl_ecom_color')->result();
}
function get_product_size_by_ids($ids){
	$ci = &get_instance();
	$ci->db->where_in('id', $ids);
	return $ci->db->get('tbl_ecom_size')->result();
}
...
if(!function_exists('echoEncodeDie'))
{
    function echoEncodeDie($status, $message)
    {
...
if(!function_exists('is_user_login'))
{
	...
	{
		$ci = &get_instance();
		$user = $ci->session->userdata('UserData');//in array
		if(!empty($user)){
			return $user;
		}else{
			return false;
...
if(!function_exists('gen_rand_id'))
{
    function gen_rand_id(){
    	$ci = &get_instance();
        $RandomDigit = mt_rand(1000000, 9999999);
        $data=$ci->db->get_where('tbl_ecom_orders', ['order_id'=>$RandomDigit])->row();
        if($data){
            gen_rand_id();
        }
        return $RandomDigit;
